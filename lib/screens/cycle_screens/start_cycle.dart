...
class _StartCroppingCycleScreenState extends State<StartCroppingCycleScreen> {
  ...
  void initState() {
    super.initState();
    _loadData();
    // Auto-generate cycle name when crop or variety changes
    cycleNameController.addListener(_updateCycleName);
  }

  void _updateCycleName() {
    final autoGenerated = _getAutoGeneratedName();
    if (cycleNameController.text.isEmpty || cycleNameController.text == autoGenerated) {
      cycleNameController.text = autoGenerated;
    }
  }
  ...
  Future<void> _fetchFarm() async {
    ...
    if (snapshot.docs.isNotEmpty) {
      ...
      selectedFarmId = snapshot.docs.first.id;
      farmName = data['name'] ?? 'Farm';
  ...
  Future<void> _fetchFields() async {
    ...
    setState(() {
      fields = snapshot.docs
          .where((doc) {
            final data = doc.data();
            // Only include fields that belong to the selected farm
            return data['farmId'] == selectedFarmId;
          })
          .map((doc) {
            final data = doc.data();
            return {
  ...
  Future<void> _saveCycle() async {
    ...
    if (_selectedCropType == null) {
      _showSnackBar('Please select a crop type.', isError: true);
      return;
    }
    if (_selectedVariety == null) {
      _showSnackBar('Please select a variety.', isError: true);
      return;
    }
  ...
  Future<void> _pickDate(bool isPlanting) async {
    ...
    setState(() {
      if (isPlanting) {
        ...
        harvestDate = date.add(const Duration(days: 62));
      } else {
        harvestDate = date;
      }
    });
  ...
  Widget build(BuildContext context) {
    // Get fields filtered by the selected farm (client-side filtering)
    final filteredFields = fields.where((field) {
      return field['farmId'] == selectedFarmId;
    }).toList();

    // Update cycle name when crop or variety changes
    final autoName = _getAutoGeneratedName();
    if (cycleNameController.text.isEmpty || 
        cycleNameController.text == autoName) {
      cycleNameController.text = autoName;
    }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            if (selectedFarmId != null && farmName != null)
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: const Color(0xFF25523B).withValues(alpha: 0.15),
                  ),
                ),
                child: Row(
                  children: [
                        ...
                        children: [
                          Text(
                            ...
                            style: TextStyle(
                              ...
                              color: const Color(0xFF25523B).withValues(alpha: 0.7),
                            ),
                          ),
                          Text(
                            farmName!,
            ...
            _TargetLocationCard(
              ...
              onFieldChanged: (id) {
                final field = filteredFields.firstWhere((f) => f['id'] == id);
            ...
            _CycleParametersCard(
              ...
              onCropTypeChanged: (value) {
                setState(() {
                  ...
                  cycleNameController.text = _getAutoGeneratedName();
                });
              },
                        ...
                        children: [
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              Text(
                                fields.isEmpty ? 'No fields found' : 'Tap map for details',
                                style: TextStyle(
                                    fontSize: 9,
...
class _CycleParametersCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final varieties = selectedCropType != null 
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDropdown(
            ...
            items: cropVarieties.keys.map((crop) {
              return DropdownMenuItem<String>(
                value: crop,
                child: Text(crop),
              );
          ...
          _buildDropdown(
            ...
            hint: selectedCropType == null ? 'Select crop type first' : 'Select variety',
            ...
            isEnabled: selectedCropType != null,
          ),
  ...
  Widget _buildDropdown({
    ...
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 14),
      decoration: BoxDecoration(
        color: isEnabled ? const Color(0xFFF5F6F3) : const Color(0xFFEEEEEE),
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: const Color(0xFFE8ECEF).withValues(alpha: 0.6),
        ),
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          hint: Text(
            hint,
            style: TextStyle(
              color: isEnabled ? const Color(0xFFA1A5A8) : Colors.grey[400],
              fontSize: 14,
            ),
          ),
          icon: Icon(
            Icons.expand_more,
            size: 20,
            color: isEnabled ? const Color(0xFF5E6266) : Colors.grey[400],
          ),
          items: items,
